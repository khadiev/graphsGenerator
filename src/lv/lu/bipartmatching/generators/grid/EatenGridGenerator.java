 ...
 */
public class EatenGridGenerator extends OneRectangleGrid {
    int[][] grid;
    int[][] d;
    int n = 0;
    int m = 0;
    ...
    public EatenGridGenerator(int havePerfectMatching) {
    ...
    private void eatRectangle(GeneratorParams params) {
        GridGeneratorParams params1 = (GridGeneratorParams)params;
        int eatenCells = params1.getEatenCellsNumber();
        int eatenWays = params1.getEatenWaysNumber();
        int eatenDeapth = params1.getEatenDeapth();


        int segment = n/eatenWays;
        int h = eatenCells/(eatenWays*eatenDeapth);
        int start = (segment - h)/2;

        for (int way = 0; way<eatenWays; way++){
            int y = way*segment + start;
            for (int i = 0; i <eatenDeapth; i++){
                for (int j = 0; j < h; j++){
                    grid[y+j+1][i+1] = 0;
                }
            }
